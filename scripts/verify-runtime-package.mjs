...
for (const required of ["dist/openclaw/index.mjs", "package.json", "openclaw.plugin.json"]) {
  if (!files.has(required)) {
    throw new Error(`npm package is missing required runtime artifact: ${required}`);
  }
}

console.log("[runtime-package] OK: dist/openclaw/index.mjs is included in npm pack output");
